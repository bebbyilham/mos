...
class Administrator extends MX_Controller
{
    ...
    public function tabelPernyataanJawaban()
    {
        $fetch_data = $this->Admin_model->make_datatables_pernyataan_target_jawaban();
        $data = array();
        $no = $_POST['start'];
        $str = 65;
        foreach ($fetch_data as $row) {
        ...
        $output = array(
            "draw"                => intval($_POST['draw']),
            "recordsTotal"        => $this->Admin_model->get_all_data_pernyataan_target_jawaban(),
            "recordsFiltered"     => $this->Admin_model->get_filtered_data_pernyataan_target_jawaban(),
            "data"                => $data
        );
        echo json_encode($output);
    }

    public function simpanpernyataanJawaban()
    {
        $data = array(
            'id_pertanyaan'            => $_POST['id_pertanyaan'],
            'jawaban'            => $_POST['jawaban'],
        ...
        $this->Admin_model->simpan_list_pernyataan_target_jawaban($data);
        echo json_encode($data);
    }

    public function getAllJawabanPernyataan()
    {
        $data = $this->Admin_model->fetch_all_jawaban_pernyataan($_POST['id_pertanyaan']);
        echo json_encode($data);
    }

    public function simpanJawabanBetulPernyataan()
    {
        $data = array(
            'jawaban_betul'            => $_POST['jawaban'],
        );

        $this->Admin_model->simpan_jawaban_betul_pernyataan($data, $_POST['id_pertanyaan']);
        echo json_encode($data);
    }

    public function getPertanyaanPernyataan()
    {
        // $data = $this->Admin_model->fetch_pertanyaan($_POST['id_pertanyaan']);
        $cek_pertanyaan = $this->db->select('
                           *
                            ')
            ->get_where('pertanyaan_target', ['pertanyaan_target.id' => $_POST['id_pertanyaan']])->row_array();
        // echo json_encode($cek_pertanyaan['jawaban_betul']);
        if ($cek_pertanyaan) {
            echo json_encode([
                'metadata' => [
    ...
    public function selfControl()
    {
        ...
        $data['content'] = '';
        $page = 'administrator/self_control';
        // echo modules::run('template/loadview', $data);
        echo modules::run('template/loadview', $data, $page);
    }

    public function tambahselfControl()
    {
        $data['title'] = 'Self Control';
        $data['user'] = $this->db->get_where('user', ['username' =>
        $this->session->userdata('username')])->row_array();
        ...
        $data['content'] = '';
        $page = 'administrator/tambah_self_control';
        // echo modules::run('template/loadview', $data);
        echo modules::run('template/loadview', $data, $page);
    }

    public function simpanselfControl()
    {
        $data = array(
            'metode'            => $_POST['metode'],
            'jenis'        => $_POST['jenis'],
        ...
        $this->Admin_model->simpan_self_control($data);
        echo json_encode($data);
    }

    public function tabelselfcontrol()
    {
        $fetch_data = $this->Admin_model->make_datatables_selfcontrol();
        $data = array();
        $no = $_POST['start'];
        foreach ($fetch_data as $row) {
            $no++;
            $sub_array = array();
            $sub_array[] = $no;
            $sub_array[] = "<b>" . $row->metode . "</b><br>" . strtoupper("$row->jenis");
            $sub_array[] = substr($row->description, 0, 100);
            if ($row->status == 'aktif') {
                $sub_array[] = '
                <div class="dropdown">
            ...
            $sub_array[] = '<span href="#" class="status badge badge-primary" title="Diunggah" >' . $row->created_at . '</span><br>' . '<span href="#" class="status badge badge-info" title="Diperbarui" >' . $row->updated_at . '</span><br>';
            $sub_array[] = '
            <a href="#" class="fa fa-images ml-2 mr-2 metode" id="' . $row->id . '" metode="' . $row->metode . '" jenis="' . $row->jenis . '" data-toggle="modal" data-target="#staticBackdrop" title="Metode"></a>
            <a href="#" class="fa fa-info-circle ml-2 mr-2 info" id="' . $row->id . '" metode="' . $row->metode . '" jenis="' . $row->jenis . '" data-toggle="modal" data-target="#staticBackdrop" title="Info"></a>
          ';
        ...
        $output = array(
            "draw"                => intval($_POST['draw']),
            "recordsTotal"        => $this->Admin_model->get_all_data_selfcontrol(),
            "recordsFiltered"     => $this->Admin_model->get_filtered_data_selfcontrol(),
            "data"                => $data
        );
        echo json_encode($output);
    }

    public function kontenmetode($id)
    {
        $data['title'] = 'Metode';
        $data['user'] = $this->db->get_where('user', ['username' =>
        $this->session->userdata('username')])->row_array();
        ...
        $data['content'] = '';
        $page = 'administrator/konten_metode';
        echo modules::run('template/loadview', $data, $page);
    }

    public function simpanlistselfcontrol()
    {
        $data = array(
            'id_selfcontrol'            => $_POST['id_metode'],
            'link'            => $_POST['link'],
            'jenis'        => $_POST['jenis'],
            'urutan'     => $_POST['urutan'],
            'status'          => $_POST['status'],
        ...
        $this->Admin_model->simpan_list_selfcontrol($data);
        echo json_encode($data);
    }

    public function tabellistselfcontrol()
    {
        $fetch_data = $this->Admin_model->make_datatables_list_selfcontrol();
        $data = array();
        $no = $_POST['start'];
        foreach ($fetch_data as $row) {
            $no++;
        ...
        $output = array(
            "draw"                => intval($_POST['draw']),
            "recordsTotal"        => $this->Admin_model->get_all_data_list_selfcontrol(),
            "recordsFiltered"     => $this->Admin_model->get_filtered_data_list_selfcontrol(),
            "data"                => $data
        );
        echo json_encode($output);
    }

    public function ubahstatuslistselfcontrol()
    {
        $data = array(
            'status'            => $_POST['status'],
        );
    ...
    public function listselfcontrolpertanyaan($id)
    {
        $data['title'] = 'Pertanyaan';
        $data['user'] = $this->db->get_where('user', ['username' =>
        $this->session->userdata('username')])->row_array();
        $data['id_list_selfcontrol'] = $id;

        $data['content'] = '';
        $page = 'administrator/list_selfcontrol_pertanyaan';
        echo modules::run('template/loadview', $data, $page);
    }

    public function simpanlistselfcontrolPertanyaan()
    {
        $data = array(
            'id_list_selfcontrol'            => $_POST['id_list_selfcontrol'],
            'pertanyaan'            => $_POST['pertanyaan'],
            'jenis'        => $_POST['jenis'],
            'urutan'     => $_POST['urutan'],
            'status'          => $_POST['status'],
        ...
        $this->Admin_model->simpan_pertanyaan_selfcontrol($data);
        echo json_encode($data);
    }

    public function tabellistselfcontrolPertanyaan()
    {
        $fetch_data = $this->Admin_model->make_datatables_selfcontrol_pertanyaan();
        $data = array();
        $no = $_POST['start'];
        foreach ($fetch_data as $row) {
            $no++;
        ...
        $output = array(
            "draw"                => intval($_POST['draw']),
            "recordsTotal"        => $this->Admin_model->get_all_data_selfcontrol_pertanyaan(),
            "recordsFiltered"     => $this->Admin_model->get_filtered_data_selfcontrol_pertanyaan(),
            "data"                => $data
        );
        echo json_encode($output);
    }

    public function ubahstatusselfcontrolpertanyaan()
    {
        $data = array(
            'status'            => $_POST['status'],
        );

        $this->Admin_model->ubah_status_selfcontrol_pertanyaan($data, $_POST['id']);
        echo json_encode($data);
    }

    public function tabelSelfcontrolJawaban()
    {
        $fetch_data = $this->Admin_model->make_datatables_self_control_jawaban();
        $data = array();
        $no = $_POST['start'];
        $str = 65;
        foreach ($fetch_data as $row) {
        ...
        $output = array(
            "draw"                => intval($_POST['draw']),
            "recordsTotal"        => $this->Admin_model->get_all_data_self_control_jawaban(),
            "recordsFiltered"     => $this->Admin_model->get_filtered_data_self_control_jawaban(),
            "data"                => $data
        );
        echo json_encode($output);
    }

    public function simpanselfcontrolJawaban()
    {
        $data = array(
            'id_pertanyaan'            => $_POST['id_pertanyaan'],
            'jawaban'            => $_POST['jawaban'],
        ...
        $this->Admin_model->simpan_list_self_control_jawaban($data);
        echo json_encode($data);
    }

    public function getAllJawabanSelfcontrol()
    {
        $data = $this->Admin_model->fetch_all_jawaban_self_control($_POST['id_pertanyaan']);
        echo json_encode($data);
    }

    public function simpanJawabanBetulSelfcontrol()
    {
        $data = array(
            'jawaban_betul'            => $_POST['jawaban'],
        );

        $this->Admin_model->simpan_jawaban_betul_self_control($data, $_POST['id_pertanyaan']);
        echo json_encode($data);
    }

    public function getPertanyaanSelfcontrol()
    {
        // $data = $this->Admin_model->fetch_pertanyaan($_POST['id_pertanyaan']);
        $cek_pertanyaan = $this->db->select('
                           *
                            ')
            ->get_where('pertanyaan_selfcontrol', ['pertanyaan_selfcontrol.id' => $_POST['id_pertanyaan']])->row_array();
        // echo json_encode($cek_pertanyaan['jawaban_betul']);
        if ($cek_pertanyaan) {
            echo json_encode([
                'metadata' => [
    ...
    public function ubahstatusselfcontroljawaban()
    {
        $data = array(
            'status'            => $_POST['status'],
        );
    ...
    public function tambahkuesioner()
    {
        ...
        $data['content'] = '';
        $page = 'administrator/tambah_kuesioner';
        // echo modules::run('template/loadview', $data);
        echo modules::run('template/loadview', $data, $page);
    }

    public function simpankuesioner()
    {
        $data = array(
            'keterangan'            => $_POST['keterangan'],
            'jenis'        => $_POST['jenis'],
        ...
        $this->Admin_model->simpan_kuesioner($data);
        echo json_encode($data);
    }

    public function tabelkuesioner()
    {
        $fetch_data = $this->Admin_model->make_datatables_kuesioner();
        $data = array();
        $no = $_POST['start'];
        foreach ($fetch_data as $row) {
            $no++;
            $sub_array = array();
            $sub_array[] = $no;
            $sub_array[] = "<b>" . $row->keterangan . "</b><br>" . strtoupper("$row->jenis");
            $sub_array[] = substr($row->description, 0, 100);
            if ($row->status == 'aktif') {
                $sub_array[] = '
                <div class="dropdown">
            ...
            $sub_array[] = '<span href="#" class="status badge badge-primary" title="Diunggah" >' . $row->created_at . '</span><br>' . '<span href="#" class="status badge badge-info" title="Diperbarui" >' . $row->updated_at . '</span><br>';
            $sub_array[] = '
            <a href="#" class="fa fa-images ml-2 mr-2 kuesioner" id="' . $row->id . '" keterangan="' . $row->keterangan . '" jenis="' . $row->jenis . '" data-toggle="modal" data-target="#staticBackdrop" title="keterangan"></a>
            <a href="#" class="fa fa-info-circle ml-2 mr-2 info" id="' . $row->id . '" keterangan="' . $row->keterangan . '" jenis="' . $row->jenis . '" data-toggle="modal" data-target="#staticBackdrop" title="Info"></a>
          ';
        ...
        $output = array(
            "draw"                => intval($_POST['draw']),
            "recordsTotal"        => $this->Admin_model->get_all_data_kuesioner(),
            "recordsFiltered"     => $this->Admin_model->get_filtered_data_kuesioner(),
            "data"                => $data
        );
        echo json_encode($output);
    }

    public function kontenkuesioner($id)
    {
        $data['title'] = 'Kuesioner';
        $data['user'] = $this->db->get_where('user', ['username' =>
        $this->session->userdata('username')])->row_array();
        ...
        $data['content'] = '';
        $page = 'administrator/konten_kuesioner';
        echo modules::run('template/loadview', $data, $page);
    }

    public function simpanlistkuesioner()
    {
        $data = array(
        ...
        $this->Admin_model->simpan_list_kuesioner($data);
        echo json_encode($data);
    }

    public function tabellistkuesioner()
    {
        $fetch_data = $this->Admin_model->make_datatables_list_kuesioner();
        $data = array();
        $no = $_POST['start'];
        foreach ($fetch_data as $row) {
            $no++;
        ...
        $output = array(
            "draw"                => intval($_POST['draw']),
            "recordsTotal"        => $this->Admin_model->get_all_data_list_kuesioner(),
            "recordsFiltered"     => $this->Admin_model->get_filtered_data_list_kuesioner(),
            "data"                => $data
        );
        echo json_encode($output);
    }

    public function ubahstatuslistkuesioner()
    {
        $data = array(
            'status'            => $_POST['status'],
        );

        $this->Admin_model->ubah_status_list_kuesioner($data, $_POST['id']);
        echo json_encode($data);
    }

    public function listkuesionerpertanyaan($id)
    {
        $data['title'] = 'Pertanyaan';
        $data['user'] = $this->db->get_where('user', ['username' =>
        $this->session->userdata('username')])->row_array();
        $data['id_list_kuesioner'] = $id;

        $data['content'] = '';
        $page = 'administrator/list_kuesioner_pertanyaan';
        echo modules::run('template/loadview', $data, $page);
    }

    public function simpanlistkuesionerPertanyaan()
    {
        $data = array(
            'id_list_kuesioner'            => $_POST['id_list_kuesioner'],
            'pertanyaan'            => $_POST['pertanyaan'],
            'jenis'        => $_POST['jenis'],
            'urutan'     => $_POST['urutan'],
            'status'          => $_POST['status'],
        ...
        $this->Admin_model->simpan_pertanyaan_kuesioner($data);
        echo json_encode($data);
    }

    public function tabellistkuesionerPertanyaan()
    {
        $fetch_data = $this->Admin_model->make_datatables_kuesioner_pertanyaan();
        $data = array();
        $no = $_POST['start'];
        foreach ($fetch_data as $row) {
            $no++;
        ...
        $output = array(
            "draw"                => intval($_POST['draw']),
            "recordsTotal"        => $this->Admin_model->get_all_data_kuesioner_pertanyaan(),
            "recordsFiltered"     => $this->Admin_model->get_filtered_data_kuesioner_pertanyaan(),
            "data"                => $data
        );
        echo json_encode($output);
    }

    public function ubahstatuskuesionerpertanyaan()
    {
        $data = array(
            'status'            => $_POST['status'],
        );

        $this->Admin_model->ubah_status_kuesioner_pertanyaan($data, $_POST['id']);
        echo json_encode($data);
    }

    public function tabelkuesionerJawaban()
    {
        $fetch_data = $this->Admin_model->make_datatables_kuesioner_jawaban();
        $data = array();
        $no = $_POST['start'];
        $str = 65;
        foreach ($fetch_data as $row) {
        ...
        $output = array(
            "draw"                => intval($_POST['draw']),
            "recordsTotal"        => $this->Admin_model->get_all_data_kuesioner_jawaban(),
            "recordsFiltered"     => $this->Admin_model->get_filtered_data_kuesioner_jawaban(),
            "data"                => $data
        );
        echo json_encode($output);
    }

    public function simpankuesionerJawaban()
    {
        $data = array(
            'id_pertanyaan'            => $_POST['id_pertanyaan'],
            'jawaban'            => $_POST['jawaban'],
        ...
        $this->Admin_model->simpan_list_kuesioner_jawaban($data);
        echo json_encode($data);
    }

    public function getAllJawabankuesioner()
    {
        $data = $this->Admin_model->fetch_all_jawaban_kuesioner($_POST['id_pertanyaan']);
        echo json_encode($data);
    }

    public function simpanJawabanBetulkuesioner()
    {
        $data = array(
            'jawaban_betul'            => $_POST['jawaban'],
        );

        $this->Admin_model->simpan_jawaban_betul_kuesioner($data, $_POST['id_pertanyaan']);
        echo json_encode($data);
    }
    ...
    public function getPertanyaankuesioner()
    {
        ...
        $cek_pertanyaan = $this->db->select('
                           *
                            ')
            ->get_where('pertanyaan_kuesioner', ['pertanyaan_kuesioner.id' => $_POST['id_pertanyaan']])->row_array();
        // echo json_encode($cek_pertanyaan['jawaban_betul']);
        if ($cek_pertanyaan) {
            echo json_encode([
                'metadata' => [
    ...
    public function ubahstatuskuesionerjawaban()
    {
        $data = array(
            'status'            => $_POST['status'],
        );

        $this->Admin_model->ubah_status_kuesioner_jawaban($data, $_POST['id']);
        echo json_encode($data);
    }
}
